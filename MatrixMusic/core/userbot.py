...
class Userbot(Client):
    ...
    async def start(self):
        LOGGER("ميــوزك ميدو").info(f"جارِ تشغيل الحساب المساعد . . .")
        if config.STRING1:
            await self.one.start()
            try:
                await self.one.join_chat("UJ_5Q")
                await self.one.join_chat("U7_4K")
            except:
                pass
            assistants.append(1)
            try:
            ...
            LOGGER("ميــوزك ميدو").info(f"تم بدء تشغيل الحساب المساعد {self.one.name} ...✓")
        ...
        if config.STRING2:
            ...
            LOGGER("ميــوزك ميدو").info(f"Assistant Two Started as {self.two.name}")
        ...
        if config.STRING3:
            ...
            LOGGER("ميــوزك ميدو").info(f"Assistant Three Started as {self.three.name}")
        ...
        if config.STRING4:
            ...
            LOGGER("ميــوزك ميدو").info(f"Assistant Four Started as {self.four.name}")
        ...
        if config.STRING5:
            ...
            LOGGER("ميــوزك ميدو").info(f"Assistant Five Started as {self.five.name}")

    async def stop(self):
        LOGGER("ميــوزك ميدو").info(f"Stopping Assistants...")
        try:
            if config.STRING1:
                await self.one.stop()
            if config.STRING2:
